...
static struct sdio_embedded_func lte_func_array[] = {
	{
		.f_class        = SDIO_CLASS_NONE,
		.f_maxblksize   = 512,
	},
...
static struct embedded_sdio_data yl_lte_emb_data = {
	.cis    = {
		.vendor         = 0x0296,
		.device         = 0x5347,
		.blksize        = 512,
#ifdef CONFIG_ARCH_OMAP3 //Deleted by huangjiefeng for support high speed mode in 2011.12.20
		.max_dtr        = 24000000,
#else
		.max_dtr        = 48000000,
#endif
...
void __init omap2_hsmmc_init(struct omap2_hsmmc_info *controllers)
{
	...
	for (c = controllers; c->mmc; c++) {
		...
		//added by huangjifeng for cp5860e
		if (c->mmc == (CONFIG_TIWLAN_MMC_CONTROLLER-1)) {
			mmc->slots[0].embedded_sdio = &yl_lte_emb_data;
			mmc->slots[0].register_status_notify =
				&yl_lte_status_register;
			mmc->slots[0].card_detect = &yl_lte_status;
		}
...
#ifdef CONFIG_TIWLAN_SDIO
			if(c->mmc == 2)
				mmc->slots[0].ocr_mask  = 0xF0000;//lte only support 3v, huangjiefeng in 20120320
			else
