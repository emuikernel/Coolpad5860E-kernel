...
static void omap_uart_restore_context(struct omap_uart_state *uart)
{
	...
	/* terry.cao@yulong.com Nov. 16th, 2011
	   Take OMAP44xx's UART DMA solution to OMAP3630
	 */
...
static inline void omap_uart_disable_clocks(struct omap_uart_state *uart)
{
	if (!uart->clocked)
        ...
        * cation is very fast.
        */
       if ((uart->num==2) && omap_uart_active(uart->num, 0))//omap_is_console_port_num(uart->num) &&
               return;
...
{
	struct omap_uart_state *uart;
	extern int smartphone_suspend;//add by yulong.huangjiefeng.2011.03.16

	list_for_each_entry(uart, &uart_list, node) {
		if (num == uart->num && uart->can_sleep) {		
            //add by yulong.huangjiefeng.2011.03.16 start
			if(0 == uart->num)
			{
            ...
            //add by yulong.huangjiefeng.2011.03.16 end
			{
                omap_uart_enable_rtspullup(uart);
                omap_uart_disable_clocks(uart);
