...
#include <plat/misc.h> /*added by xiao for wl1271 power mamager 2010-5-6*/
extern void power_wl1271(int on);
extern struct wl1271_state_t wl1271_state;/*added by xiao 2010-5-10*/
...
/*added by xiao 2010-5-10*/
void init_wl1271_state(void)
{
	wl1271_state.wifi_state=0;
	wl1271_state.bt_state=0;
	wl1271_state.WIFI_1V8=0;
	spin_lock_init(&(wl1271_state.lock_for_wl1271));
//	spin_lock_init(&(wl1271_state.lock_for_bt));
}
#if CONFIG_WL12XX_LDO
#define WIFI_LDO_PM_MASTER 0x0e
#define WIFI_LDO_PM_RECEIVER_REG 0x22
/*
*   init and set  value of ldo output voltage to 1.8v, NOTE: we should enable this ldo pin
*   by xiaoxiangyun, 2010-4-9
*/
void regulator_power_for_wl1271(void)
{
	int ret=0;
	printk("regulator_power_for_wl1271 by xiao\n");

	twl_i2c_write_u8(TWL4030_MODULE_PM_MASTER, 0xc0, WIFI_LDO_PM_MASTER);
	twl_i2c_write_u8(TWL4030_MODULE_PM_MASTER, 0x0c, WIFI_LDO_PM_MASTER);

	ret=twl_i2c_write_u8(TWL4030_MODULE_PM_RECEIVER,0x01,WIFI_LDO_PM_RECEIVER_REG);//added by xiao
	if(ret)
	{
	   printk("1c_write for wl1271 error by xiao\n");
	   return;
	}
	ret=twl_i2c_write_u8(TWL4030_MODULE_PM_MASTER, 0x00, WIFI_LDO_PM_MASTER);
	if(ret)
	{
	   printk("2c_write for wl1271 error by xiao\n");
	   return;
	}

	return;
}
EXPORT_SYMBOL(regulator_power_for_wl1271);
#endif
...
static int __init zoom2_wifi_init(void)
{
	...
	init_wl1271_state();/*added by xiao 2010-5-10*/
#if CONFIG_WL12XX_LDO
	regulator_power_for_wl1271();//added by xiaoxiangyun 2010-4-24
#endif
