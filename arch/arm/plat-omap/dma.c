...
#define OMAP_DMA_RD_ACTIVE	    (0x01 << 9)
#define OMAP_DMA_WR_ACTIVE	    (0x01 << 10)
#define OMAP_DMA_RDWR_ACTIVE	(0x03 << 9)
...
int omap_get_dma_rd_active_status(int lch)
{
	u32		ccr;

	ccr = dma_read(CCR(lch));
	return (ccr & OMAP_DMA_RD_ACTIVE);
}
EXPORT_SYMBOL(omap_get_dma_rd_active_status);

int omap_get_dma_wr_active_status(int lch)
{
	u32		ccr;

	ccr = dma_read(CCR(lch));
	return (ccr & OMAP_DMA_WR_ACTIVE);
}
EXPORT_SYMBOL(omap_get_dma_wr_active_status);
...
int omap_get_dma_rd_wr_active_status(int lch)
{
	...
	ccr = dma_read(CCR(lch));
	return (ccr & OMAP_DMA_RDWR_ACTIVE);
}
EXPORT_SYMBOL(omap_get_dma_rd_wr_active_status);

void omap_print_status(void)
{
	int ch=1;
	for(ch=0;ch<32;ch++)
	{
		printk(KERN_ERR "ch:%d,state;%x\n",ch,dma_read(CCR(ch)) & (OMAP_DMA_CCR_EN|OMAP_DMA_RDWR_ACTIVE)) ;
	}
