...
static int soc_codec_close(struct snd_pcm_substream *substream)
{
	...
	if (!codec->active) {
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			...
			//modified play DTMF no sound when calling, by yuanyufang 2010-12-27
...
if(substream->stream == SNDRV_PCM_STREAM_CAPTURE || (substream->stream == SNDRV_PCM_STREAM_PLAYBACK && (play_prepare++==0)))
{
...
else printk("fengchunsong do nothing %d\n",play_prepare);
...
static int soc_suspend(struct device *dev)
{
	...
	* suspend before that's finished - wait for it to complete.
	...
	for (i = 0; (i < codec->num_dai) && (smartphone_calling_enable==0) ; i++) {
...
/**
 * snd_soc_info_volsw2 - single mixer info callback
 * @kcontrol: mixer control
 * @uinfo: control element information
 *
 * Callback to provide information about a single mixer control.
 ...
 * Returns 0 for success.
 */
int snd_soc_info_volsw2(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_info *uinfo)
{
	struct soc_mixer_control *mc =
		(struct soc_mixer_control *)kcontrol->private_value;
...
EXPORT_SYMBOL_GPL(snd_soc_info_volsw2);

/**
 * snd_soc_get_volsw2 - single mixer get callback
 * @kcontrol: mixer control
 * @ucontrol: control element information
 *
 * Callback to get the value of a single mixer control.
 ...
 * Returns 0 for success.
 */
int snd_soc_get_volsw2(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct soc_mixer_control *mc =
		(struct soc_mixer_control *)kcontrol->private_value;
...
EXPORT_SYMBOL_GPL(snd_soc_get_volsw2);

/**
 * snd_soc_put_volsw2 - single mixer put callback
 * @kcontrol: mixer control
 * @ucontrol: control element information
 *
 * Callback to set the value of a single mixer control.
 ...
 * Returns 0 for success.
 */
int snd_soc_put_volsw2(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct soc_mixer_control *mc =
		(struct soc_mixer_control *)kcontrol->private_value;
...
EXPORT_SYMBOL_GPL(snd_soc_put_volsw2);
