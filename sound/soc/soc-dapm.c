...
//added by yuanyufang,  2010-02-02
#include <mach/gpio.h>
#include <mach/mux.h>
...
extern int headset_amp_en(int enable);
extern int speaker_amp_en(int enable);
...
static int dapm_update_bits(struct snd_soc_dapm_widget *widget)
{
	...
	//forbit to disable TWL4030_HSMICBIAS_EN in running, by yuanyufang, 2010-12-27
	if(widget->id == snd_soc_dapm_micbias)
	{
		if(widget->shift == 2 && power == 0)
		    return 0;
	}
	
	//added by yuanyufang, 2010-12-27
	if ( (widget->id == snd_soc_dapm_mixer || widget->id == snd_soc_dapm_micbias)
		  && (power == 0) 
		  && (smartphone_calling_enable == 1))
	{
		//yl_debug("for calling and suspend in %s func\n",__FUNCTION__);
		return 0;
	}
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (widget->event) {
		...
		//added by yuanyufang , to clear pop noise, 2010-12-27
		if(widget->id == snd_soc_dapm_switch)
		{		    
            #ifdef CONFIG_SWITCH_GPIO
			...
			{
				//gpio_value = gpio_get_value(GPIO_97);
				if(cp5860e_speaker_ic == ALC108_IC)
				{
					gpio_value = get_alc108_earphone();
				}
				else
				{
					gpio_value = get_wm9093_earphone();
				}
				if(gpio_value == 1)
					headset_amp_en(0);
			}
			...
			{
				//gpio_value = gpio_get_value(GPIO_22);
				if(cp5860e_speaker_ic == ALC108_IC)
				{
					gpio_value = get_alc108_speaker();
				}
				else
				{
					gpio_value = get_wm9093_speaker();
				}
				if(gpio_value == 1)
					speaker_amp_en(0);
			}		
		...
		//added by yuanyufang , to clear pop noise, 2010-12-27
		if(widget->id == snd_soc_dapm_switch && gpio_value == 1)
		{
			if(earphone_insert == 0)	//earphone inserted
			{
				headset_amp_en(1);
