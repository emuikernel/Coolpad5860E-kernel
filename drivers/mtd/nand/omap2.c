...
static int omap_wait(struct mtd_info *mtd, struct nand_chip *chip)
{
	...
	int val;	// for TI patch(wangwenbiao) 

	if (state == FL_ERASING)
		timeo += msecs_to_jiffies(1000);		/*timeo += (HZ * 400) / 1000;*/	
	else
		timeo += msecs_to_jiffies(100);		/*timeo += (HZ * 20) / 1000;*/
	...
	while (time_before(jiffies, timeo)) {
		...
		val = __raw_readb(this->IO_ADDR_R);
	...
	if (status == NAND_STATUS_FAIL) {
		printk (KERN_WARNING " ***omap_wait: timeout...\n");
		WARN_ON(1);
	}
