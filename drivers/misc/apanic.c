...
static unsigned int phy_offset_1( unsigned int offset)
{
	unsigned int logic_block = offset>>17;
	...
	return offset + ((phy_block-logic_block)<<17);
}
...
static void mtd_panic_notify_add(struct mtd_info *mtd)
{
	...
	if (!proc_entry_created || !hdr->console_length || !hdr->threads_length)
	{
		printk(KERN_ERR "mtd_panic_erase:%d,%d,%d\n",proc_entry_created , hdr->console_length,hdr->threads_length);
		mtd_panic_erase();
	}
			...
			void *ptr)
{
	...
	rc = apanic_writeflashpage(ctx->mtd,phy_offset_1(0), ctx->bounce);
	if (rc <= 0) {
