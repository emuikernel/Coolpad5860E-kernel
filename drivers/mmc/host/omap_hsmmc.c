...
//test for sync failed
#include <linux/blkdev.h>
struct omap_hsmmc_host *locol_host = NULL;
//end
#ifdef CONFIG_PM
...
#ifdef CONFIG_MACH_CP5860E
#include <mach/cp5860e_lte_modem.h>//added by huangjiefeng
#endif
...
#define OMAP_MMC_REQ_TIMEOUT        10000//10s, added by huangjiefeng in 2012.0604
...
struct omap_hsmmc_host {
	...
	struct	work_struct	mmc_carddetect_work;
        /*when card detect pin status is different with card status,invoke this delay_work*/
	...
	struct timer_list req_tout_timer;//added by huangjiefeng
};
...
static int omap_hsmmc_reg_get(struct omap_hsmmc_host *host)
{
	...
	{
		reg = regulator_get(host->dev, "vio");
		if (IS_ERR(reg)) {
			printk("%s: mmc%d\n", __func__, host->id);
			...
			*/
			...
			mmc_slot(host).ocr_mask = 0xf0000;//lte only support 3v, huangjiefeng in 2012.03.20
			
			printk("%s: mmc%d, ocm_mask=0x%4x\n",__func__, host->id, mmc_slot(host).ocr_mask);

			/* Allow an aux regulator */
			reg = regulator_get(host->dev, "vio_aux");
	...
	else
	{
		...
		if (IS_ERR(reg)) {
			...
			*/
			...
			mmc_slot(host).ocr_mask = mmc_regulator_get_ocrmask(reg);
			//if(host->id == 0)
				//mmc_slot(host).ocr_mask = 0xf0080;
			printk("%s: mmc%d, ocm_mask=0x%4x\n",__func__, host->id, mmc_slot(host).ocr_mask);
	...
	struct mmc_data *data)
{
	...
	yl_sd_debug(LOG_INFO, "%s: CMD%d, argument 0x%08x\n",
		mmc_hostname(host->mmc), cmd->opcode, cmd->arg);
	host->cmd = cmd;
					...
					struct mmc_request *mrq)
{
	...
	if(host->id == 1)
		del_timer(&host->req_tout_timer);//added by huangjiefeng
...
static void omap_hsmmc_do_irq(struct omap_hsmmc_host *host, int status)
{
	...
	if (status & ERR) {
		//printk(KERN_WARNING"%s: mmc%d error\n", __func__, host->id);
		yl_sd_debug(LOG_WARNING, "%s: mmc%d error\n", __func__, host->id);
#ifdef CONFIG_MMC_DEBUG
...
//test for sync fail 
enum blk_eh_timer_return deal_with_blocking_expection(struct request *req)
{
	struct mmc_data *data;
	int end_cmd = 0, end_trans = 0;
	struct omap_hsmmc_host *host = locol_host;
	printk(KERN_ERR "mmc: Excute request time out.\n");
...
EXPORT_SYMBOL_GPL(deal_with_blocking_expection);
//end
/*
...
static void card_status_work(struct work_struct *work)
{
    struct omap_hsmmc_host *host =
		container_of(work, struct omap_hsmmc_host, status_work);
    struct omap_mmc_slot_data *slot = &mmc_slot(host);
    int carddetect;
    struct mmc_host *mmc=host->mmc;
    if (slot->card_detect)
		carddetect = slot->card_detect(host->dev, host->slot_id);
   if(carddetect != mmc->card_status)
     {
        mmc_detect_change(host->mmc, (HZ * 50) / 1000);
        schedule_delayed_work(&host->status_work,(HZ * 200) / 1000);
     } 
	yl_sd_debug(LOG_INFO, "carddetect=%d,mmc->card_status=%d\n",carddetect,mmc->card_status);      
}
/*
...
static void omap_hsmmc_detect(struct work_struct *work)
{
	...
	yl_sd_debug(LOG_INFO, "SD detect irq occur\n");
	if (carddetect)
...
static irqreturn_t omap_hsmmc_cd_handler(int irq, void *dev_id)
{
	...
	printk(KERN_ERR "ISR for handling card insertion and removal. %s \r\n",__func__);
...
static void omap_hsmmc_request(struct mmc_host *mmc, struct mmc_request *req)
{
	...
	if((host->id == 1) && get_lte_sdio_cmd_tout_flag())
	{
		printk(KERN_ERR"%s:lte sdio cmd timeout\n", __func__);
	...
	if(host->id == 2)//added for workarownd wifi crash with clock off by huangjiefeng in 20120712
	{
		if((!(omap_readl(0x48004a00) & (1 << 30))) ||(!(omap_readl(0x48004a10) & (1 << 30))))
		{
			printk(KERN_ERR"%s:wifi sdio clk is off\n", __func__);
	...
	*/
	...
	if ((host->id == CONFIG_TIWLAN_MMC_CONTROLLER-1)/*||(host->id == CONFIG_TIWLAN_MMC_CONTROLLER-2)*/) {
	...
	if(host->id == 1)
		mod_timer(&host->req_tout_timer,
				(jiffies + msecs_to_jiffies(OMAP_MMC_REQ_TIMEOUT)));//added by huangjiefeng
...
//added by huangjiefeng in 2012.06.04
void  omap_lte_sdio_req_tout_timer_hdlr(unsigned long host_data)
{
	struct omap_hsmmc_host *host = (struct omap_hsmmc_host *)host_data;
	struct mmc_request *mrq;
	...
	{
		printk(KERN_ERR"%s: mmc%d+\n", __func__, host->id);
		host->response_busy = 0;
	}
	
	mrq = host->mrq;
	if(NULL == mrq || NULL == mrq->cmd)
	{
		printk(KERN_ERR"%s: mrq or mrq->cmd is null\n", __func__);
	}
	else
	{
	...
	if(NULL == host->cmd)
	{
		printk(KERN_ERR"%s: host->cmd is null\n", __func__);
	}
	else
		host->cmd->error = -ETIMEDOUT;

	lte_sdio_cmd_timeout(1);

	if (host->data)
		omap_hsmmc_dma_cleanup(host, -ETIMEDOUT);

	omap_hsmmc_reset_controller_fsm(host, SRD|SRC);
...
static void omap_hsmmc_status_notify_cb(int card_present, void *dev_id)
{
       ...
       yl_sd_debug(LOG_INFO, "%s: card_present %d\n", mmc_hostname(host->mmc),
		card_present);
...
//end
	...
	//added by huangjiefeng for support lte
	if (pdev->id == CONFIG_TIWLAN_MMC_CONTROLLER-2) {
			if (pdata->slots[0].embedded_sdio != NULL) {
				mmc_set_embedded_sdio_data(mmc,
				&pdata->slots[0].embedded_sdio->cis,
				&pdata->slots[0].embedded_sdio->cccr,
	...
	INIT_WORK(&host->mmc_carddetect_work, omap_hsmmc_detect);
        if ((mmc_slot(host).card_detect_irq)){
            yl_sd_debug(LOG_INFO,"host id is %d*******\n\n\n\n",host->id);
            INIT_DELAYED_WORK(&host->status_work,card_status_work);
             }
	...
	//added by huangjifeng in 20110920
	if (mmc_slot(host).register_status_notify) {
		if (pdev->id == CONFIG_TIWLAN_MMC_CONTROLLER-2) {
			mmc_slot(host).register_status_notify(
				omap_hsmmc_status_notify_cb, host);
		}
	...
	if(host->id == 1)
		setup_timer(&host->req_tout_timer, omap_lte_sdio_req_tout_timer_hdlr, (unsigned long)host);//added by huangjiefeng
...
static int omap_hsmmc_suspend(struct device *dev)
{
	...
	if(1 == host->id)//added by huangjiefeng, 
		return 0;
	...
	if (host) {
		...
		if(mmc_slot(host).card_detect_irq)
		      cancel_delayed_work_sync(&host->status_work);
		mmc_host_enable(host->mmc);
	...
	if((1 == host->id) && (ret == 0))//added by huangjiefeng in 20120313
	{
		set_sdio_resume_status(0);
	}
...
static int omap_hsmmc_resume(struct device *dev)
{
	...
	if(host->id == 1)//added by huangjiefeng
		return 0;
	...
	if((1 == host->id))//added by huangjiefeng in 20120313
	{
		set_sdio_resume_status(1);
	}
