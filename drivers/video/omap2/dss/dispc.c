...
static void dispc_enable_lcd_out(enum omap_channel channel, bool enable)
{
	...
	printk(KERN_ERR"+ enable=%d is_on=%d\n",enable,is_on);
#if 1
	if (!enable && is_on) {
		init_completion(&frame_done_completion);
		printk(KERN_ERR"Registering FD IRQ\n");
	...
	if (!enable && is_on) {
		...
		printk(KERN_ERR"Unregistering the FD IRQ\n");
...
void dispc_close_irqenable(void)
{
enable_clocks(1);
dispc_write_reg(DISPC_IRQENABLE,0X00);
enable_clocks(0);
}
