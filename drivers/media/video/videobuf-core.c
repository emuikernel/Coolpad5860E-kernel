...
void videobuf_queue_cancel(struct videobuf_queue *q)
{
	...
	int i;
        dprintk(1, "*************enter videobuf_queue_cancel A**************");
	q->streaming = 0;
	q->reading  = 0;
	...
	for (i = 0; i < VIDEO_MAX_FRAME; i++) {
		if (NULL == q->bufs[i])
			continue;
		if (q->bufs[i]->state == VIDEOBUF_QUEUED) {
	    dprintk(1,"*************enter videobuf_queue_cancel B**************");	
	    if(q->bufs[i]->queue.prev != 0x200200 && q->bufs[i]->queue.next != 0x100100) {
	    	printk(KERN_ERR "*************enter videobuf_queue_cancel c**************");	
			list_del(&q->bufs[i]->queue);  //avoid null pointer
			} else {
			  dprintk(1, "*************enter videobuf_queue_cancel d**************");	
			}
