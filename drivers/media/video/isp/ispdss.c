...
{
	if (dev_ctx.opened) {
		CAM_DBG("dss: get_resource: device is already opened\n");
		return -EBUSY;
	}

	dev_ctx.isp = isp_get();
	if (!dev_ctx.isp) {
		CAM_DBG("dss: Can't get ISP resource\n");
		return -EACCES;
	}
	dev_ctx.opened	= 1;
...
{
	struct isp_device *isp_res = NULL;
	int i = 0;

    if(dev_ctx.isp == NULL)
		return;

    isp_res=dev_get_drvdata(dev_ctx.isp);
		 ...
		 u32 in_phy_add, u32 in_off)
{
	...
	if (output_buffer_index >= dev_ctx.num_video_buffers) {
		CAM_DBG("ouput buffer index is out of range %d", output_buffer_index);
		return -EINVAL;
	}

	if (dev_ctx.config_state != STATE_CONFIGURED) {
		CAM_DBG("State not configured \n");
		return -EINVAL;
	}
	...
	if (!dev_ctx.out_buf_virt_addr[output_buffer_index]) {
		...
		if (IS_ERR_VALUE(
			dev_ctx.out_buf_virt_addr[output_buffer_index])) {
			CAM_DBG("Mapping of output buffer failed"
						"for index \n");
			return -ENOMEM;
		}
	}
	...
	if (!dev_ctx.in_buf_virt_addr[input_buffer_index]) {
		...
		if (IS_ERR_VALUE(
			dev_ctx.in_buf_virt_addr[input_buffer_index])) {
			CAM_DBG("Mapping of input buffer failed"
						"for index \n");
			return -ENOMEM;
		}
	}
