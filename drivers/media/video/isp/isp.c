...
static struct vcontrol {
	...
	{
		{
			.id = V4L2_CID_PRIVATE_OMAP3ISP_ISP_SLOWDOWN,
			.type = V4L2_CTRL_TYPE_BOOLEAN,
			.name = "SLOWDOWN ISP",
			.minimum = 0,
			.maximum = 1,
					       ...
					       int out_h)
{
	...
	if (in_w > 0 && in_h > 0 && out_w > 0 && out_h > 0) {
		...
		if (out_size > in_size) {
			ratio = DIV_ROUND_UP(out_size,  in_size);
			clamp_t(int, ratio, 0,
...
int isp_g_ctrl(struct device *dev, struct v4l2_control *a)
{
	...
	case V4L2_CID_PRIVATE_OMAP3ISP_CSI2MEM:
		...
		break;
	case V4L2_CID_PRIVATE_OMAP3ISP_ISP_SLOWDOWN:
		slowdownIsp = 1;
		rval = 0;
		break;
	default:
...
int isp_s_ctrl(struct device *dev, struct v4l2_control *a)
{
	...
	case V4L2_CID_PRIVATE_OMAP3ISP_CSI2MEM:
		...
		break;
	case V4L2_CID_PRIVATE_OMAP3ISP_ISP_SLOWDOWN:
		slowdownIsp = new_value ? true : false;
		rval = 0;
		break;
	default:
		rval = -EINVAL;
