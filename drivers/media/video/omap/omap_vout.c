...
//printk(KERN_ERR "wwwwwwwwwwwwww\n");
	if (vout->isprsz & ISPRSZ_ENABLE)
		return;

	if (vout->isprsz & ISPRSZ_MANUALMODE)
        {
           if (!(vout->win.w.width < 180 && vout->win.w.height < 180 ))
		return;
        }
	...
	if (scale >= 4096 || (scale > 2048 && src_w > 1024))
		goto enable_and_exit;
	//if ((vout->pix.height  >= 480) || (vout->pix.width>= 480))
		//goto enable_and_exit;
	return;
...
{
	vrfb_configured = 0; //zhanglin 20120106
...
#if 0
	v4l2_dbg(1, debug, &vout->vid_dev->v4l2_dev,
		"%s vout : %p, enable=%d addr=%x width=%d\n height=%d color_mode=%d\n"
		"rotation=%d mirror=%d posx=%d posy=%d out_width = %d \n"
		"out_height=%d rotation_type=%d screen_width=%d\n",
		__func__, vout, info.enabled, info.paddr, info.width, info.height,
			  ...
			  struct videobuf_buffer *vb)
{
  ...
  spin_lock(&vout->vbq_lock); //Tushar add
	...
	spin_unlock(&vout->vbq_lock); //Tushar add
}
...
#ifdef CONFIG_OMAP3_ISP_RESIZER_ON_OVERLAY
        printk(KERN_ERR "enable_isp_rsz in new place\n");
	enable_isp_rsz(vout);
#endif
		...
		int vid_num)
{
	...
	printk("KRANTI :static_vrfb_allocation is %d\n",static_vrfb_allocation);
...
static int __init omap_vout_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < vid_dev->num_displays; i++) {
		...
		if (dssdrv && dssdrv->get_update_mode &&
