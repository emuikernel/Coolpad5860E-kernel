...
#define R_PB_CFG            0x14
#define R_PB_WORD_MSB     	0x15
#define R_PB_WORD_LSB       0x16
...
void __init twl4030_power_init(struct twl4030_power_data *twl4030_scripts)
{
	...
	if (err)
	 ...
	 /*add patch from wenbiao for out of  warm reset 2012.2.29 */
     err = twl_i2c_write_u8(TWL4030_MODULE_PM_MASTER, 2, R_PB_CFG);
     if (err)
      	pr_err("TWL4030 Failed to set PB_CFG\n");

     err = twl_i2c_write_u8(TWL4030_MODULE_PM_MASTER, 0xfe, R_PB_WORD_MSB);
     if (err)
    	pr_err("TWL4030 Failed to set PB_WORD_MSB\n");

     err = twl_i2c_write_u8(TWL4030_MODULE_PM_MASTER, 0x7e, R_PB_WORD_LSB);
     if (err)
     	pr_err("TWL4030 Failed to set PB_WORD_LSB\n");
    /* end patch*/
