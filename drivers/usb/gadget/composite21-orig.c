...
static struct wake_lock gadget_wake_lock;
		...
		struct usb_function *function)
{
    ...
    printk("adding '%s'/%p to config '%s'/%p\n",
...
static int adjust_config_bNumInterfaces(const void *buf, int length) {
    int n;
    int bNumInterfaces = 0;
    struct usb_config_descriptor *c = (struct usb_config_descriptor *)buf;
    struct usb_descriptor_header *h;

    if(c->bDescriptorType != USB_DT_CONFIG)
        return 0;

    for(h = (struct usb_descriptor_header *)buf, n = 0; n < length;
            h = (struct usb_descriptor_header *)(buf + n)) {
        if(h->bDescriptorType == USB_DT_INTERFACE) {
            bNumInterfaces ++;
        }
        n += le16_to_cpu(h->bLength);
    }
		...
		enum usb_device_speed speed, void *buf, u8 type)
{
	...
	list_for_each_entry(f, &config->functions, list) {
        ...
        if(strcmp(f->name,"gser0")==0 || strcmp(f->name,"gser1")==0)
        {
            if(f->disabled == 1)
                bserialhide = 1;
            else
        ...
        if((strcmp(f->name,"adb") == 0) && (descriptors[0] != NULL))
        {
            if(bserialhide == 1){
                ((struct usb_interface_descriptor*) descriptors[0])->bInterfaceNumber = 3;
            }
            else{
                ((struct usb_interface_descriptor*) descriptors[0])->bInterfaceNumber = 5;
            }
        }
		...
		for (; f != config->interface[interfaceCount];)
			interfaceCount++;
#else
        if (descriptors[0] == NULL || f->disabled) {
            c->bNumInterfaces--;
            continue;
        }
#endif
    ...
    /* if many associated interfacces are in the configuration, it will cause
     * a incorrect value of bNumInterfaces, so we need check all interface
     * descriptors again as one configuration buf is filled, and give out a
     * right bNumInterfaces */
    status = adjust_config_bNumInterfaces(buf, len);
    if(c->bNumInterfaces != status) {
        DBG(config->cdev, "check bNumInterfaces %d actually %d\n",
                c->bNumInterfaces, status);
        c->bNumInterfaces = status;
    }
...
unknown:
        ...
        /* for VIA Telecom CBP's specific Control Requests */
        if (((ctrl->bRequestType & USB_TYPE_MASK) == USB_TYPE_VENDOR) &&
	        ...
	        for (i = 0; i < cdev->config->next_interface_id; i ++) {
	        ...
	        if (i == cdev->config->next_interface_id)
		        f = NULL;
        }
...
int __init usb_composite_register(struct usb_composite_driver *driver)
{
    ...
    wake_lock_init(&gadget_wake_lock, WAKE_LOCK_SUSPEND, "usb_gadget");
...
void __exit usb_composite_unregister(struct usb_composite_driver *driver)
{
    ...
    composite->unbinded = 1;
    for(i=1;i<= composite->function_count.counter;i++)
        device_destroy(composite->class, MKDEV(0, i));
		
	class_destroy(composite->class);
	usb_gadget_unregister_driver(&composite_driver);

    wake_lock_destroy(&gadget_wake_lock);
}
