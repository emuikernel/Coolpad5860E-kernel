...
#define ATTR_OLD_ENABLE	1
#define ATTR_STATISTICS	2
#define ATTR_NUPS		3
#define ATTR_NDOWNS		4
#define ATTR_UPSIZE		5
#define ATTR_DOWNSIZE	6
#define ATTR_SPLIT		7
#define ATTR_DTR		8
...
static ssize_t rawbulk_attr_store(struct device *dev, struct device_attribute *attr,
	...
	if (idx <= ATTR_OLD_ENABLE) {
		int enable;
		if (idx == ATTR_ENABLE) {
			enable = simple_strtoul(buf, NULL, 10);
		} else if (idx == ATTR_OLD_ENABLE) {
		...
		printk(KERN_INFO "%s - tid %d enable %d activated %d\n", __func__,
				fn->transfer_id, enable, fn->activated? 1: 0);
...
static int rawbulk_create_files(struct rawbulk_function *fn) {
    int n, rc;
    for (n = 0; n < fn->max_attrs; n ++){
		if (n == ATTR_OLD_ENABLE)
			continue;
        rc = device_create_file(&fn->dev, &fn->attr[n]);
        if (rc < 0) {
            printk(KERN_ERR "%s - failed to create file %s on %s.\n", __func__,
                fn->attr[n].attr.name, dev_name(&fn->dev));
            while (--n >= 0) {
				if (n == ATTR_OLD_ENABLE)
					continue;
...
int rawbulk_register_tty(struct rawbulk_function *fn) {
    ...
    dev = tty_register_device(rawbulk_tty_driver, fn->transfer_id, NULL /* &fn->dev */);
    if (IS_ERR(dev))
...
void rawbulk_unregister_tty(struct rawbulk_function *fn) {
	tty_unregister_device(rawbulk_tty_driver, fn->transfer_id);
}
...
static void query_cp_dtr_complete(struct urb *urb) {
	struct rawbulk_function *fn = rawbulk_get_by_index(RAWBULK_TID_MODEM);
	if (urb->status < 0)
		return;
	if (urb->actual_length == 1)
		fn->dsr_state = *((unsigned char *)urb->transfer_buffer);
	printk(KERN_ERR "%s=>fn->dsr_state=0x%x\n",__func__,fn->dsr_state);
}

static void query_cp_dtr(struct work_struct *data) {
	struct rawbulk_function *fn = rawbulk_get_by_index(RAWBULK_TID_MODEM);
	/*struct usb_ctrlrequest ctrl = {
		.bRequestType = USB_DIR_IN | USB_TYPE_VENDOR, //0x40
		.bRequest = 0x02,
		.wValue = 0,
		.wLength = 1,
		.wIndex = 0,
	...
	//rawbulk_async_control_msg(&ctrl, query_cp_dtr_complete);
	struct usb_device *cp = rawbulk_get_cp();
	if (cp) {
		int ret = usb_control_msg(cp, usb_rcvctrlpipe(cp, 0), 0x02,
				USB_DIR_IN | USB_TYPE_VENDOR, 0x00, 0,
				&fn->dsr_state, 1, USB_CTRL_SET_TIMEOUT);
		if (ret < 0)
			printk("%s - %d\n", __func__, ret);
		rawbulk_put_cp(cp);
	}
	...
	if (rawbulk_check_enable(fn))
		schedule_delayed_work(&fn->query_work, 3*HZ);
}
...
static void __init init_rawbulk_container(struct rawbulk_function *fn,
    ...
    switch (transfer_id) {
	    ...
	    INIT_DELAYED_WORK(&fn->query_work, query_cp_dtr);
            break;
    ...
    add_device_attr(fn, ATTR_OLD_ENABLE, fn->shortname, 0755);
    ...
    if (transfer_id == RAWBULK_TID_MODEM) {
        add_device_attr(fn, ATTR_DTR, "dtr", 0744);
        fn->max_attrs ++;
    }
...
static struct attribute *_old_sysfs[_MAX_TID + 1] = { NULL };
static struct attribute_group _old_sysfs_group = {
	.attrs = _old_sysfs,
};
struct kobject *_old_sysfs_stuff;
...
static int __init init(void) {
    ...
    for (n = 0; n < _MAX_TID; n ++) {
		...
		_old_sysfs[n] = &fn->attr[ATTR_OLD_ENABLE].attr;
    }

	/* make compatiable with old bypass sysfs access */
	_old_sysfs_stuff = kobject_create_and_add("usb_bypass", NULL);
	if (_old_sysfs_stuff) {
		rc = sysfs_create_group(_old_sysfs_stuff, &_old_sysfs_group);
		if (rc < 0)
			printk(KERN_ERR "failed to create old bypass stuff, but continue...\n");
	}
