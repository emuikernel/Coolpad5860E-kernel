...
+ * Author: Roger Quadros <roger.quadros@...>
+ *
...
static void gc_rx_complete(struct usb_ep *ep, struct usb_request *req)
{
	...
	case 0:
		/* normal completion */
		//i = kfifo_get(&gc->rx_fifo, req->buf, req->actual);
...
static int gc_do_tx(struct gc_dev *gc)
{
	...
	while (!list_empty(pool)) {
		...
		//len = kfifo_put(&gc->tx_fifo, req->buf, len);
...
static int gc_do_rx(struct gc_dev *gc)
{
	...
	int			started = 0;
	/*<  check gc and gchar before accessing whether it has valid address or not or it is freed when usb is reset/unplugged*/
	if(NULL == gc)
		return -EINVAL;
	else if(NULL == gc->gchar)
		return -EINVAL;
	else
	{
		pool	= &gc->rx_pool;
...
static int gc_open(struct inode *inode, struct file *filp)
{
	...
	if (!gc->tx_fifo_buf && gc->gchar->ep_in) {
		printk(KERN_INFO "USB:MTP:gc_open:__Setup Function ---\n");
		gc->tx_fifo_buf = vmalloc(GC_BUF_SIZE);
		if (gc->tx_fifo_buf == NULL)
		{
			printk(KERN_INFO "USB:MTP:gc_open:1. __Setup Function ---\n");
			return -ENOMEM;
		}		
			kfifo_init_gadget(&gcdata.gcdevs[minor].tx_fifo,
				gc->tx_fifo_buf, GC_BUF_SIZE);
//		gcdata.gcdevs[minor].tx_fifo =	kfifo_init(gc->tx_fifo_buf, GC_BUF_SIZE, GFP_KERNEL,&(gc->tx_lock));
	//	gcdata.gcdevs[minor].tx_fifo =	*(kfifo_init(gc->tx_fifo_buf, GC_BUF_SIZE, GFP_KERNEL,&(gc->tx_lock)));
	}
//gc->tx_fifo 
...
//			kfree(gc->tx_fifo_buf);
			vfree(gc->tx_fifo_buf);
			return -ENOMEM;
		}
	//	gcdata.gcdevs[minor].rx_fifo =	*(kfifo_init(gc->rx_fifo_buf, GC_BUF_SIZE, GFP_KERNEL,&(gc->rx_lock)));
		//gcdata.gcdevs[minor].rx_fifo =	kfifo_init(gc->rx_fifo_buf, GC_BUF_SIZE, GFP_KERNEL,&(gc->rx_lock));
		kfifo_init_gadget(&gcdata.gcdevs[minor].rx_fifo,
				...
				size_t len, loff_t *o)
{
	...
	if (len) {
		...
		if (!read) {
			...
			if(gc->gchar->mtp_read_status==ECANCELED)
			{
			printk("MTP_USB: Device Request Cance,mtp_read_status is %d ",gc->gchar->mtp_read_status);
			gc->gchar->mtp_read_status=0;
			gc->gchar->mtp_write_status=0;
			gc->gchar->mtp_read=0;
			return ECANCELED;
			}
			
                        /* sleep till we have some data */
                        if (wait_event_interruptible(gc->rx_wait,gc_can_read(gc)))
							return -ERESTARTSYS;                       	
                }
		
		 read = kfifo_to_user(&gc->rx_fifo, buff, len,&total);
        }
						...
						size_t len, loff_t *o)
{
	...
	if (!gc->gchar || !gc->gchar->ep_in) {
		printk(KERN_INFO "USB:MTP:gc_write:2. In Write function of u_char driver %d\n",EINVAL);
		/* not yet connected or writing not possible */
		return -EINVAL;
	}

	gc->gchar->mtp_write=1;
	...
	if (len) {
		if (kfifo_is_full(&gc->tx_fifo)) {
			...
			printk(KERN_INFO "USB:MTP:gc_write:Inside the kfifo is full \n");
		...
		wrote = kfifo_from_user(&gc->tx_fifo, buff, len,&total);
		if (wrote < 0)
	...
	if(gc->gchar->mtp_write_status==ECANCELED)
		{
		printk("MTP_USB: Device Request Cance,mtp_read_status is %d ",gc->gchar->mtp_write_status);
		gc->gchar->mtp_write_status=0;
		gc->gchar->mtp_read_status=0;
		gc->gchar->mtp_write=0;
		return ECANCELED;
		}
	gc->gchar->mtp_write=0;
	return total;
...
fail2:
	if (gchar->ep_in) {
		...
		printk(KERN_INFO "USB:MTP:gchar_connect: Disabling ep_in \n");
