...
static struct f_rndis * _f_rndis;
...
rndis_iad_descriptor  = {
	...
	.bFunctionClass =	0xE0,//USB_CLASS_COMM,
	.bFunctionSubClass =	1,//USB_CDC_SUBCLASS_ACM,
	.bFunctionProtocol =	3,//USB_CDC_PROTO_NONE,
	/* .iFunction =		DYNAMIC */
};
...
static struct usb_ether_platform_data rndis_pdata = {
	...
	.vendorID       = 0x1EBF,
	.vendorDescr    = "Yulong Coolpad Inc.",
	};
//#endif
...
rndis_bind(struct usb_configuration *c, struct usb_function *f)
{
    ...
    f->descriptors = eth_fs_function;//null_rndis_descs;//eth_fs_function;
    f->hs_descriptors = eth_hs_function;//null_rndis_descs;//eth_hs_function;
...
//	if (rndis_pdata) {
		...
		//if (rndis_set_param_vendor(rndis->config, 0x1EBF,
...
int rndis_function_bind_config(struct usb_configuration *c)
{
	...
	snprintf(device_serial, MAX_USB_SERIAL_NUM, "%08X%08X%08X%08X", 0x23F20002, 0x9FFC0000, 0x0160B317, 0x0F00C020);

	printk("KKKKKKKEEEEEEEEEE->device_serial=%s\n",device_serial);
...
{
	struct f_rndis *rndis = _f_rndis;

	if(&rndis->port)
	{
		printk(KERN_INFO "USB:%s rndis_function => (%s)\n", __func__,
		...
		if (enable) {
			...
			rndis->port.func.descriptors = null_rndis_descs;
			rndis->port.func.hs_descriptors = null_rndis_descs;
		}
	}
	else
		printk("[%s] rndis->port does not exist\n", __func__);
}
