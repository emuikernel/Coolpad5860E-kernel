...
int hcd_bus_suspend(struct usb_device *rhdev, pm_message_t msg)
{
	...
	if (!hcd->driver->bus_suspend) {
        ...
        if(strcmp("usb1",kobject_name(&(rhdev->dev.kobj)))==0){
            if(msg.event & PM_EVENT_AUTO)
                g_usb_autosuspended = 1;
            printk(KERN_ERR "%s==>g_usb_autosuspended=%d\n",__func__,g_usb_autosuspended);
        }
	}
	if (status == 0) {
...
void usb_hcd_resume_root_hub_ex (struct usb_hcd *hcd)
{
	unsigned long flags;
	struct usb_device *udev = hcd->self.root_hub;
...
EXPORT_SYMBOL_GPL(usb_hcd_resume_root_hub_ex);
