...
#include "../../power/twl4030_bci_reg.h"
...
#define DRIVER_NAME "twl4030_usb"
wait_queue_head_t usb_wait_q;//hey add @10-8-26
wait_queue_head_t usb_irq_q;
bool g_setupoccured = false;
bool g_usbirqoccured = false;
charger_status g_charger_type = charger_unplug;
int charge_timeout = 30*HZ;
int g_linkedonce = 0;
...
static void twl4030_phy_power(struct twl4030_usb *twl, int on)
{
	...
	if (on) {
		...
		printk(KERN_ERR "%s=>pwr=%x\n",__func__,twl4030_usb_read(twl,PHY_PWR_CTRL));
	} else  {
	    msleep(250);
		pwr |= PHY_PWR_PHYPWD;
		WARN_ON(twl4030_usb_write_verify(twl, PHY_PWR_CTRL, pwr) < 0);
...
int test_charge_function(int enable, void *_twl)
{
	...
	if((enable) && (status != USB_EVENT_NONE))
	{
		twl4030_phy_resume(twl);
        printk(KERN_INFO "USB:test_charge_function:enable_usb_mass_storage set USB_COMPOSITE\n");
	...
	else
	{
        ...
        printk(KERN_INFO "USB:test_charge_function:enable_usb_mass_storage set USB_CHARGE_OLNY\n");
		enable_usb_mass_storage = 0;
	}
...
extern void charge_interrupt_done(int insert_device);
int battery_init_finish = 0;
...
static irqreturn_t twl4030_usb_irq(int irq, void *_twl)
{
	...
	int insert_device=charger_unplug;

    printk(KERN_ERR "enter %s\n",__func__);
	//hardware_version = get_device_hardware_version();
	hardware_version = 2;
	status = twl4030_usb_linkstat(twl);
	if (status >= 0) {
		...
		{
			twl4030_phy_suspend(twl, 0);
            printk(KERN_ERR "USB:twl4030_usb_irq:disconnect\n");
			g_setupoccured = 0;//hey add @10-8-26
			g_charger_type = charger_unplug;
			charge_interrupt_done(insert_device);
			yl_uart_switch(YL_UART_SWITCH_AP);
		}
		else
		{
			yl_usb_switch(YL_USB_SWITCH_AP);
			g_charger_type = usb_charge;
			g_usbirqoccured = 1;
			wake_up_interruptible(&usb_irq_q);
			if(battery_init_finish == 1)
			{
				insert_device = usb_charge;//twl4030_identify_charge_device();
				//if(insert_device == usb_charge)
				{
					twl4030_phy_resume(twl);
				}
				charge_interrupt_done(insert_device);
            ...
            printk(KERN_ERR "USB:twl4030_usb_irq:USB_LINK_VBUS\n");
			enable_usb_mass_storage = 1;//add by yanghaishan,100828
		}
		
		blocking_notifier_call_chain(&twl->otg.notifier, status,
				twl->otg.gadget);//can'n be deleted huangjiefeng
	}
	sysfs_notify(&twl->dev->kobj, NULL, "vbus");
...
{
	twl4030_usb_irq(g_twl->irq, g_twl);
}
...
static int charger_type_check(void *data)
{
	int charge_device;
	while(1){
		printk(KERN_INFO "USB:charger_type_check:waitting usb irq occur!\n");
		wait_event_interruptible(usb_irq_q,g_usbirqoccured == 1);
		printk(KERN_INFO "USB:charger_type_check: usb irq occured!\n");

		g_usbirqoccured = 0;

		if(wait_event_interruptible_timeout(usb_wait_q,g_setupoccured ==1,charge_timeout) == 0)
		{
			charge_device = twl4030_identify_insert_device();
			if(!(charge_device == charger_unplug))
			{
				printk(KERN_INFO "USB:charger_type_check:TIMEOUT\n");
				g_charger_type = ac_charge;
				charge_interrupt_done(ac_charge);
			}
			
		}
		else
			printk(KERN_INFO "USB:charger_type_check:usb reset occured!\n");
...
static int __devinit twl4030_usb_probe(struct platform_device *pdev)
{
	...
	init_waitqueue_head(&usb_irq_q);
	pThread = kthread_create(charger_type_check,NULL,"charger_check");

	if(pThread != NULL)
		wake_up_process(pThread);
	else
		printk(KERN_ERR "USB:twl4030_usb_probe:create charger_detect thread fail!\n");
	...
	g_twl = kzalloc(sizeof *twl, GFP_KERNEL);
	g_twl = twl;//add by huangjiefeng
