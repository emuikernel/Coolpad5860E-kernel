...
static int serial_open(struct tty_struct *tty, struct file *filp)
{
	...
	if(serial->disconnected)
	{
	    dbg("=====tang enter %s serial->disconnected %d \n", __func__, serial->disconnected);
		return -ENODEV;
	}
...
static void port_release(struct device *dev)
{
	...
	//if (!IS_ERR((void *)(&port->write_fifo)) && port->write_fifo)
...
extern int cbp_rawbulk_intercept(void *private_data, int enable);
#else
extern int option_rawbulk_intercept(void *private_data, int enable);
#endif
...
#ifdef CONFIG_USB_SERIAL_VIATELECOM_CBP
    return rawbulk_bind_host_interface(serial->interface, cbp_rawbulk_intercept, (void *)serial);
#else
    return rawbulk_bind_host_interface(serial->interface, option_rawbulk_intercept, (void *)serial);
#endif
#else
    return 0;
#endif
