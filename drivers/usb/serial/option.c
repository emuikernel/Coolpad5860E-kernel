...
void option_disconnect(struct usb_serial *serial);
...
static int usb_serial_option_suspend(struct usb_interface *intf, pm_message_t message)
{
    ...
    //rawbulk_suspend_host_interface(intf->cur_altsetting->desc.bInterfaceNumber, message);
    return 0;
}
...
static int usb_serial_option_resume(struct usb_interface *intf)
{
    ...
    //rawbulk_resume_host_interface(intf->cur_altsetting->desc.bInterfaceNumber);
    return 0;
}
/* End of temp added */
...
extern void sleep_modem(void);
 ...
 int option_rawbulk_intercept(void *private_data, int enable) {
    int rc = 0;
    int n;
...
void option_disconnect(struct usb_serial *serial) {
#ifdef CONFIG_USB_ANDROID_RAWBULK
    rawbulk_unbind_host_interface(serial->interface);
#endif
    usb_wwan_disconnect(serial);
}
