...
static void ed_halted(struct ohci_hcd *ohci, struct td *td, int cc)
{
	...
	default:
		ohci_dbg (ohci,
			"urb %p path %s vid/pid %04x/%04x ep%d%s %08x cc %d --> status %d\n",
			urb, urb->dev->devpath,
			urb->dev->descriptor.idVendor,
			urb->dev->descriptor.idProduct,
			usb_pipeendpoint (urb->pipe),
			usb_pipein (urb->pipe) ? "in" : "out",
			hc32_to_cpu (ohci, td->hwINFO),
			cc, cc_to_error [cc]);
		if (usb_pipeendpoint(urb->pipe) == 0 && urb->setup_packet) {
			struct usb_ctrlrequest *req = urb->setup_packet;
			ohci_dbg (ohci, "urb setup: bRequestType %02x bRequest %02x wValue %04x wIndex %04x wLength %04x\n",
