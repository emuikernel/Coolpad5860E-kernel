...
static int uart_number = 0;//added by huangjiefeng in 2011.12.20
int get_uart_number(void)
{
	return uart_number;
}
...
static void serial_omap_stop_rx(struct uart_port *port)
{
	...
	if(!port->suspended && (((up->pdev->id) == UART1) 
	        || ((up->pdev->id) == UART2)))//fengchunsong modify.uart don't have hardware cts.2011-03-23
...
receive_chars(struct uart_omap_port *up, unsigned int *status)
{
    ...
    if(up->port.state == NULL || up->port.state->port.tty==NULL)
    {
        printk(KERN_ERR "NULL pointer return in %s\n",__FUNCTION__);
        return;
    }
	...
	do {
		...
		if (unlikely(lsr & UART_LSR_BRK_ERROR_BITS)) {
			printk(KERN_WARNING"uart_num=%d\n", up->pdev->id);
			uart_number = up->pdev->id;
			/*
			 * For statistics only
			 */
			if (lsr & UART_LSR_BI) {
...
void	 serial_omap_peer(struct uart_port *port)
{
	struct uart_omap_port *up = (struct uart_omap_port *)port;
	...
	if (iir & UART_IER_RLSI) {
		if (!up->use_dma) {
			...
			if (serial_omap_start_rxdma(up) != 0)
				if (lsr & (UART_LSR_DR | UART_LSR_BI)){
					if((up->use_dma))//使用dma不读数据
				    {
			            printk(KERN_ERR"use dma,so no need read data in fifo!,id=%d,name=%s\n",up->pdev->id,up->pdev->name);
				    }
				    else
				    {
						printk(KERN_ERR"%s: it should not come here, id=%d\n", __func__, up->pdev->id);
						receive_chars(up, &lsr);
					}
				}
		}
...
static int serial_omap_startup(struct uart_port *port)
{
	 ...
	 */
	if (((up->pdev->id) == UART1) || ((up->pdev->id) == UART2))//fengchunsong modify.uart don't have hardware cts.2011-03-23
	...
	up->ier = UART_IER_RDI;//Modified by huangjiefeng in 2011.01.26
	serial_out(up, UART_IER, up->ier);
			...
			struct ktermios *old)
{
	...
	up->fcr = UART_FCR_R_TRIG_00 | UART_FCR_T_TRIG_01 |
...
int omap_uart_cts_wakeup_event(int uart_no, int state)
{
	...
	if(uart_no!=2)
	 printk(KERN_ERR "uart_no=%d,state=%d in %s @ %d\n",uart_no,state,__FUNCTION__,__LINE__);
...
int omap_uart_cts_wakeup(int uart_no, int state)
{
	...
	if (state) {
		...
		#if 0 //fengchunsong.modify.2011-9-19
		/*
		 * Enable the CTS for module level wakeup
		 */
		serial_out(up, UART_OMAP_WER,
				serial_in(up, UART_OMAP_WER) | 0x1);
		#endif //fengchunsong.modify.2011-9-19
	} else {
