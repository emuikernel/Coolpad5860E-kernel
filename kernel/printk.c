...
static int __init console_setup(char *str)
{
	...
	if (str[0] >= '0' && str[0] <= '9') {
		...
		//strncpy(buf, str, sizeof(buf) - 1);//fengchunsong change for ota update.2011-03-25
		strcpy(buf, "ttyO");
		strncpy(buf + 4, str+4, sizeof(buf) - 5);
	}
	buf[sizeof(buf) - 1] = 0;
...
void suspend_console(void)
{
	...
	if (!console_suspend_enabled)
		return;
	printk(KERN_ERR"Suspending console(s) (use no_console_suspend to debug)\n");
	acquire_console_sem();
	console_suspended = 1;
	up(&console_sem);
}
